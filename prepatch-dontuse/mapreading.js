...
class Settings {
    ...
    save() {
        localStorage.setItem('prepatch-settings', JSON.stringify(this));
...
class LHMap {
    ...
    set potshit(value) {
        ...
        localStorage.setItem("prepatch-potshit", this.__global_potshit);
    }
    ...
    set defendershit(value) {
        ...
        localStorage.setItem("prepatch-defendershit", this.__global_defendershit);
    }
    constructor(map) {
        this.__global_potshit = parseInt(localStorage.getItem("prepatch-potshit")) || 0;
        this.__potshit = 0;
        this.__global_defendershit = parseInt(localStorage.getItem("prepatch-defendershit")) || 0;
        ...
        function createPots() {
            ...
            while (potspaceavailable && potsplaced < 5) {
                ...
                if (tries > 30 && potsplaced < 5) {
                    if (createNewPot(4, 4, 3, false)) { potsplaced++; } else if (createNewPot(4, 4, 2, false)) { potsplaced++; } else if (createNewPot(4, 4, 1, false)) { potsplaced++; } else { potspaceavailable = false; }
                }
                tries++;
            }
        }
